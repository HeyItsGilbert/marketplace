...
A collection of plugins that extend [Claude Code](https://claude.ai/code) with custom skills.
...
claude plugin add /path/to/marketplace/plugins/release-manager
```
...
```bash
# Install all plugins
claude plugin add https://github.com/your-user/marketplace

# Install a single plugin
claude plugin add https://github.com/your-user/marketplace/plugins/release-manager
```

Once installed, skills are available in any Claude Code session. Type the skill name (e.g. `/release`) or describe what you want and Claude will activate the matching skill automatically.
...
1. Create `plugins/<your-plugin>/.claude-plugin/plugin.json`:
   ...
   ```
2. Create `plugins/<your-plugin>/skills/<your-skill>/SKILL.md` with YAML frontmatter (`name`, `description`) and the skill prompt as the markdown body.
