...
2. **Classify the change.** Use SemVer: major for incompatible published-interface changes, minor for backward-compatible features, and patch for backward-compatible fixes. When evidence is ambiguous, state the evidence and ask for the release decision. Complete when a single version is selected.
3. **Update release records.** Add or update the version and a dated changelog entry under an Unreleased section. Use only Keep a Changelog categories appropriate to the release type. Complete when every released manifest version and changelog entry agree.
4. **Validate the release material.** Read the final changelog section and manifests, confirm the version format and allowed categories, and run the project's applicable validation. Complete when the observed output supports the release contents.
