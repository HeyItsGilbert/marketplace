...
description: >-
  Ready-to-use Pester 5 test patterns and recipes for PowerShell module testing.
  Use when you need patterns for mocking (file system, REST APIs, credentials,
  DSC resources), when troubleshooting VS Code hangs from Pester runs, or when
  asking about testing best practices for PowerShell modules.
  Trigger phrases: "pester pattern", "mock file system", "mock REST API",
  "pester test template", "how do I test", "write a Pester test", "pester hangs",
  "VS Code freezes pester", "mock credential", "test DSC resource",
  "parametrized tests", "pester recipe", "test template", "mock example".
  DO NOT USE FOR: debugging failing builds, general Pester syntax questions,
  running tests (use pester-run), reviewing test quality (use pester-review).
allowed-tools:
  - PowerShell(*)
---
